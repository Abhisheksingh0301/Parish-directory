...
/**
 * One number carrying a spreadsheet's thousands separators: a leading group of
 * one to three digits, then two or more groups of two or three. Both Indian
 * grouping ("9,48,93,25,288") and Western ("9,876,543,210") match; a genuine
 * comma-separated pair of ten-digit numbers cannot, because its first group is
 * ten digits long.
 */
const DIGIT_GROUPED = /^\d{1,3}(?:,\d{2,3}){2,}$/;
...
function listProblem(value, who) {
  /*
   * Digit grouping, which is the other thing a spreadsheet does to a long
   * number in a column that is not formatted as text.
   *
   * "9,48,93,25,28,89,00,00,00,000" is one number wearing Indian lakh-crore
   * separators, not ten numbers. Split on commas it becomes ten, and the
   * complaint that comes back — "10 mobile numbers, keep it to 3" — sends
   * somebody looking for nine numbers that were never there. Caught before
   * the split, and named, the same way scientific notation is.
   *
   * A real pair cannot be mistaken for this: two numbers are ten digits each,
   * and the pattern needs a leading group of three digits or fewer followed
   * by at least two more groups of two or three.
   */
  const trimmed = String(value ?? '').trim();
  if (DIGIT_GROUPED.test(trimmed)) {
    const whose = who ? ` for ${who}` : '';
    return `The mobile number "${trimmed}"${whose} has had thousands separators put ` +
      'into it by a spreadsheet, which happens when the Mobile column is not ' +
      'formatted as text. Format that column as Text, type the number in again, ' +
      'and separate a second number from it with a comma.';
  }
