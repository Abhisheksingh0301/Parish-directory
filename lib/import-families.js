...
/**
 * A parish's own place in the book, read off the sheet.
 *
 * Not a reject when it will not parse — "2nd" or a stray note in the column
 * is worth ordering past rather than worth holding the whole family back
 * for, so an unreadable cell is simply no sort order, the same as a blank
 * one. See bySortOrder in models/family.js for what a missing one does.
 */
...
function readRow(row, map, lineNumber) {
   ...
   * book, and nothing in the sheet would say so afterwards.
   *
   * Each message names its column, so "that is not an email address" against a
   * line number says which cell to go and look at.
   */
  const who = get('member_name') || familyRef;

  const badEmails = addresses.listProblem(get('emails'), who);
  if (badEmails) errors.push(`Email: ${badEmails}`);
  ...
  return {
    ...
    family: {
      sort_order: parseSortOrder(get('sort_order')),
      family_id: familyRef,
...
/**
 * Everything on a row that describes the person rather than the household.
 *
 * Used to tell a row that is about somebody from a row that only corrects the
 * family's own columns — see the head adoption in `groupFamilies`. The dates
 * are the parsed day/month pairs, so a row carrying only a year that was
 * dropped does not count as being about a person.
 */
const MEMBER_VALUE_FIELDS = [
  'relation', 'dob_day', 'dob_month', 'dom_day', 'dom_month',
  'mobile', 'blood_group', 'qualification', 'occupation', 'emails'
];

function saysSomethingAboutAPerson(member) {
  return MEMBER_VALUE_FIELDS.some((field) => {
 ...
 *
 * ── The head who is not in the Member column ───────────────────────────────
 * A row is kept as a member only if it names one, and that dropped almost
 * every household in the Parish's own sheet. Its first row for a family reads
 *
 *     Sort Order  Family ID  Head of family   Member  Relation  ...  Mobile
 *     2           P008       Mr. Easow T V            Head           +919545999967
 *
 * The name is written once, in Head of family, and the Member column is left
 * empty — which is how anybody fills in a sheet whose first row is obviously
 * about the head. The row was then discarded for having no member name, and
 * every value on it that belongs to a person went with it: the mobile number,
 * the date of birth, the blood group, the qualification, the occupation. The
 * family imported with a head and no members, and the office looked at an
 * entry with no telephone number in it and no reason given.
 *
 * So where a family ends up with nobody in it, and its first row named a head
 * and said something about a person, that row becomes the head's own member
 * row. The reverse has always been done — a sheet with members and no Head of
 * family takes the head's name from the first member (see `updateFamily` and
 * `createFamily`, reason 'head-adopted'); this is the same courtesy in the
 * other direction.
 *
 * Two things it deliberately will not do.
 *
 * It invents nobody. A row that names a head and says nothing whatever about
 * a person is a family-level correction — a sheet sent to fix addresses, which
 * the Import page offers as its own workflow — and inventing a member from it
 * would add a person the office never mentioned. `saysSomethingAboutAPerson`
 * is that line.
 *
 * It only fills an empty household. A family whose rows do name members is
 * left exactly as the sheet describes it, so a head listed among them is not
 * duplicated by this.
 */
function groupFamilies(rows) {
  ...
  for (const row of rows) {
    ...
    for (const [field, value] of Object.entries(row.family)) {
      // sort_order is a number and may legitimately be 0; the blank-cell
      // check below every other field relies on is wrong for it, so it is
      // merged on its own right after this loop.
      if (field === 'sort_order') continue;
      if (!family[field] && value) family[field] = value;
    }
    if (family.sort_order == null && row.family.sort_order != null) {
      family.sort_order = row.family.sort_order;
    }
  ...
  for (const family of families.values()) {
    ...
    if (!first || !family.head_name) continue;
    if (first.member.name) continue;
    if (!saysSomethingAboutAPerson(first.member)) continue;

    family.members.push({ ...first.member, name: family.head_name });
    // Reported on the page rather than done quietly: the office asked for one
    // person on the sheet and is getting one it did not spell out.
    family.member_adopted = { line: first.line, name: family.head_name };
  }
...
const FAMILY_SHEET_FIELDS = [
  'head_name', 'address', 'hometown', 'home_parish', 'prayer_group'
];
 ...
 * something in it overwrites. Sort Order follows the same rule, once its own
 * parsing: a row where it will not parse is a family whose order is left as
 * it was, not one moved to the back of the book.
 *
...
async function updateFamily({ churchId, existing, family, line, dryRun, adjusted }) {
  ...
  values.sort_order = family.sort_order != null ? family.sort_order : current.sort_order;
  ...
  if (!dryRun) {
    await Family.update(churchId, current.id, {
      ...
      // The family's own email address, for the same reason as the photograph
      // above: the sheet stopped carrying a column for it, and `update` writes
      // every field it is given. Handing it back keeps an import from quietly
      // emptying the address a household signs in with.
      email: current.email,
      members
    });
  }
...
async function runImport({ churchId, churchName, families, dryRun = false, onExisting = 'skip' }) {
  ...
  for (const family of families) {
    ...
    /*
     * The head who was named in Head of family and not in Member, and whose
     * row would otherwise have been dropped along with the telephone number
     * on it. Decided in `groupFamilies`; reported here, where every other
     * thing the import did on the office's behalf is reported.
     */
    if (family.member_adopted) {
      adjusted.push({
        reason: 'member-adopted',
        family_id: family.family_id,
        line: family.member_adopted.line,
        message: `${family.family_id}: the Member column was empty, so ` +
                 `"${family.member_adopted.name}" was taken from Head of family ` +
                 'and that row’s mobile number, dates and other details kept'
      });
    }
