...
const ROLES = {
  ...
  editor: {
    rank: 3,
    label: 'Editor',
    blurb: 'Can add, edit and delete families. Cannot manage users or settings.'
  },
  viewer: {
    rank: 2,
    label: 'Viewer',
...
function requireAuth(req, res, next) {
  ...
  /*
   * `req.session` is gone, not merely empty, when lib/idle.js has just expired
   * this request's session — express-session deletes it on destroy. There is
   * nowhere to note where they were going in that case, and nothing is lost by
   * it: they are about to be told they were signed out and sent to the page
   * they were last on is not where they would want to resume anyway.
   */
  if (req.session && req.method === 'GET' && req.accepts('html')) {
    req.session.returnTo = req.originalUrl;
  }

  // Why they are looking at the sign-in page, rather than letting it appear
  // that the directory simply forgot them. See views/auth/login.ejs.
  return res.redirect(req.idleSignOut ? '/login?timeout=1' : '/login');
}
