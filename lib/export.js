 ...
 *
 * `Sort Order` is new for the same reason `Area / Unit` was: a parish that
 * has set one is using it to print the book, and an export that could not
 * reproduce it would not be a copy of the parish's own data.
 */
function headings(labels, withPhoto) {
  const columns = [
    labels.diocese, labels.zone, 'Church', 'Sort Order', 'Family ID', 'Head of family',
