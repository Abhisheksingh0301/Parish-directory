...
/**
 * The family's own columns, minus the Family ID, which every row carries.
 * Taken from lib/import-columns.js rather than restated, so the export cannot
 * come to disagree with the importer about which columns belong to the family.
 */
const FAMILY_ONLY = new Set(
  columns.FAMILY_FIELDS.filter((field) => field !== 'family_id')
);
...
/**
 * Every column, in the order the entry itself reads — which is the order
 * lib/import-columns.js declares, and not a second list beside it.
 *
 * This is the whole of what "the import and the export are the same file"
 * means in code. The headings are that module's headings, the order is its
 * order, and a field added or renamed there appears here without anybody
 * remembering to come and do it. A parish downloads Families (CSV), edits it
 * in Excel and uploads the same file on the Import families page: nothing is
 * renamed on the way, nothing is reordered, and no column is reported back as
 * one the importer does not recognise.
 *
 * Two columns exist here that the template has no use for, and both are
 * deliberately outside the shared block:
 *
 *   The church columns come first, and only when the export spans more than
 *   one church — a super administrator's installation-wide sheet has to say
 *   which parish a row belongs to. A single church's own export omits them
 *   entirely, because that is the file that has to go back in, and the
 *   importer loads into the church the office is signed in to.
 *
 *   `Photograph` stays appended at the end, so turning photographs on does not
 *   move every column before it. It names the image inside the zip; the
 *   importer ignores it, which is right — photographs come back through
 *   Import photographs, not through the sheet.
 */
function headings(labels, { withPhoto = false, withChurch = false } = {}) {
  return [
    ...(withChurch ? [labels.diocese, labels.zone, 'Church'] : []),
    ...columns.headerRow(),
    ...(withPhoto ? ['Photograph'] : [])
  ];
}

/**
 * One family as CSV lines — one per member, with the Family ID repeated down
 * the household and the family's own columns written once, on its first row.
 *
 * That shape is not a saving of bytes; it is the shape the importer reads. A
 * family's own columns are taken from the first row carrying its Family ID
 * (see `groupFamilies` in lib/import-families.js), so writing them once is
 * writing exactly what will be read back. It is also how the Parish's own
 * spreadsheet is laid out, which is the sheet this format was matched to.
 *
 * A family with no members still gets its one row, or it would vanish out of
 * its own export.
 */
function familyRows(church, family, photoName) {
  ...
  return members.map((m, index) => {
    ...
    const cells = columns.FIELDS.map((field) => {
      // The Family ID is on every row: it is what ties the rows together.
      if (field === 'family_id') return values.family_id;
      // The rest of the family's own columns are written once and then left
      // blank, exactly as the importer reads them back.
      if (FAMILY_ONLY.has(field)) return index === 0 ? values[field] : '';
      return member[field] ?? '';
    });

    return csv.row([
      ...(church ? [church.diocese_name, church.zone_name || '', church.name] : []),
      ...cells,
      ...(photoName === null ? [] : [index === 0 ? (photoName || '') : ''])
    ]);
  });
}
...
async function writeRows(write, churchIds, { labels, includeDrafts = true, photoName = null } = {}) {
  /*
   * One church's export is the file that goes back in, so it carries only the
   * importer's own columns. More than one church cannot: a row would not say
   * which parish it came from, and an installation-wide sheet is a record
   * rather than something to re-upload.
   */
  const withChurch = churchIds.length > 1;

  await write(BOM);
  await write(csv.row(headings(labels, { withPhoto: !!photoName, withChurch })));
  ...
  for (const churchId of churchIds) {
    ...
    for (const family of families) {
      ...
      for (const line of familyRows(withChurch ? church : null, family, photoName ? name : null)) {
        await write(line);
      }
    }
  }
