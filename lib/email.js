...
function problem(value) {
  ...
  /*
   * Two addresses where one was asked for.
   *
   * This is the mistake somebody makes on purpose: they have a second address
   * for the household and they put it in the Email column, separated by a
   * comma, because that is what the Emails column beside it accepts. Judged
   * as one address it merely "has a space in it", which is true, useless, and
   * says nothing about the column it should have gone in. Caught before the
   * whitespace check for exactly that reason — the space is the symptom.
   *
   * The family's Email cannot hold two: it is the household's sign-in name,
   * and an account cannot have two of those.
   */
  if (/[,;]/.test(address) || (address.match(/@/g) || []).length > 1) {
    return `${quoted} looks like more than one address. A family has one Email — ` +
      'it is the household\'s sign-in name. A member\'s other addresses go in the ' +
      'Emails column, which takes up to three.';
  }
