 ...
 *
 * A space, not " - ". The dash was doing a separator's job between two halves
 * that no reader was going to run together, and in the members table — nine
 * columns on one sheet of A4 — it cost three characters in each of two
 * columns on every row.
 *
 * It is safe to change in the one place it is written. Nothing parses this
 * back apart from the spreadsheet importer, and lib/import-dates.js splits on
 * whitespace as readily as on a dash, so a book exported, edited and loaded
 * again still reads its own dates.
 */
