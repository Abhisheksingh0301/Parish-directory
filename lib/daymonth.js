...
/**
 * "1975-08-02", the value a date input wants. Empty unless the date is
 * complete and real — a day and month with no year, or a 29 February an old
 * record kept in a non-leap year, has no place in a date picker.
 */
function toISO(day, month, year) {
  if (!format(day, month)) return '';

  const y = Number(year);
  if (!Number.isInteger(y) || y <= 0) return '';
  if (Number(day) > daysIn(Number(month), y)) return '';

  const pad = (n, width) => String(n).padStart(width, '0');
  return `${pad(y, 4)}-${pad(Number(month), 2)}-${pad(Number(day), 2)}`;
}
...
/**
 * A date of birth as a date picker sends it — "1975-08-02". Blank stays blank,
 * because a date of birth may not have been collected yet. Everything else is
 * checked as if it had been typed: a hand-made POST is not a date picker.
 */
function parseISO(raw, label) {
  const text = String(raw ?? '').trim();
  const empty = { day: null, month: null, year: null };

  if (!text) return { ...empty, error: null };

  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(text);
  if (!match) return { ...empty, error: `${label} is not a valid date.` };

  return parseFull(match[3], match[2], match[1], label);
}
